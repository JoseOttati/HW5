 ...
 ********************************************************************/
 ...
 class BloomFilter {
     ...
     public boolean contains(String s) {
        for (int i = 0; i < noHashes; i++) {  // Use noHashes to loop over hash functions
            long hc = hashCode(s, i);  // Use the existing hashCode method
            int bitNo = (int) (hc) & this.hashMask;  // Mask to get the correct bit position
            if (!data.get(bitNo)) {  // Check if the bit is set
                return false;  // If any bit is not set, it's definitely not in the set
            }
        }
        return true;  // All bits are set, item might be in the set
    }
